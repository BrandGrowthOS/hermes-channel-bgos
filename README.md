...
- **In-app voice calls (v0.14.0)** — the adapter serves BGOS's native WebRTC voice control plane (`voice_rpc` mint / consult / dispatch, spec §6.2): call sessions are minted directly against OpenAI Realtime with the agent's persona + recent chat context baked in, mid-call consults run as real agent turns on the same session as text chat (the agent remembers the call), and async dispatches run detached with the outcome pushed to the user's Work Stream. A call carries the caller's own OpenAI key on the mint frame (`payload.openaiApiKey`), so a Home of Agents call spends THEIR OpenAI credits and needs no gateway key; `BGOS_OPENAI_API_KEY` (or `OPENAI_API_KEY`) in the gateway env is only a fallback for a standalone host — see Configuration.
