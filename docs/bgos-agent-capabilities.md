...
**Voice calling (capability #11) — SHIPPED (plugin v0.14.0 + backend 2026-07-05+).** The plugin has no pairing, so `voice_rpc` frames arrive on the `assistant:<id>` room and replies go over the dual-auth REST routes with X-API-Key. Mint requires `BGOS_OPENAI_API_KEY` (or `OPENAI_API_KEY`) in the plugin's `.mcp.json` env; consults arrive as `[voice_consult]` channel notifications answered via the `voice_consult_reply` MCP tool (short speakable answer, ~30 s budget); dispatches arrive as `voice_task_dispatch` → `[voice_dispatch]` notifications answered EXACTLY ONCE via `complete_voice_task`. Full detail in capability #11's channel-availability paragraph.
