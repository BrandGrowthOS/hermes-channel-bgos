...
- **Mid-call consults arrive in your normal session.** When the voice model needs the agent's memory/tools/reasoning, it calls the channel-side consult tool (`hermes_agent_consult` / `openclaw_agent_consult` / `claude_agent_consult`) with args `{question: string, context?: string, responseStyle?: string}`. The channel host turns that into a regular turn on the SAME session as text chat (`bgos:<chat_id>`): a user message prefixed `[voice consult] ` + the question, followed by a `Call context: …` block when `context` is set and an `Answer style: …` block when `responseStyle` is set. Reply like any normal turn, but keep it concise and speakable, your final response text is returned to the voice model and summarized aloud.
...
- **Voice persona/instructions are configured host-side, not per-call by the agent.** The realtime preamble, persona, model id, and voice name live in the channel host's config (Hermes: `BGOS_VOICE_MODEL` / `BGOS_VOICE_VOICE` / `BGOS_VOICE_PERSONA` env, or the profile's hermes config; OpenClaw: the gateway's realtime talk config; Claude Code: `BGOS_VOICE_MODEL` / `BGOS_VOICE_VOICE` / `BGOS_VOICE_PERSONA` in the plugin's `.mcp.json` env, plus the required `BGOS_OPENAI_API_KEY`/`OPENAI_API_KEY` mint key). The agent never mints, holds, or manages voice credentials, client secrets are ephemeral by construction.

**Channel availability:** OpenClaw implements the full control plane (mint / consult / dispatch). Hermes voice is SPEC'D but its `voice_rpc.py` is not yet built (see the design doc §6.2) — do not promise calls on Hermes until it lands. The **Claude Code MCP channel implements the full control plane too** (plugin v0.14.0+ with a backend from 2026-07-05 or later): the plugin has no pairing, so the backend delivers `voice_rpc` on the `assistant:<id>` room (mint eligibility = the plugin socket being CONNECTED) and the plugin replies over the dual-auth REST routes with its X-API-Key. Claude-specific notes: **(a) mint** happens on the agent host directly against OpenAI (`POST /v1/realtime/client_secrets`) and requires `BGOS_OPENAI_API_KEY` (or `OPENAI_API_KEY`) in the plugin env — without it, calls fail with a descriptive "voice not configured" error while chat keeps working; **(b) consults** arrive as a `[voice_consult]` channel notification with a consult id — answer by calling the `voice_consult_reply` MCP tool FIRST with `consult_id` + a short speakable answer (1–3 sentences, no other tools unless strictly required; you have ~30 s — a busy session will usually blow the budget, which degrades to a graceful spoken "still working" line, and a late `voice_consult_reply` is redirected to normal chat); **(c) dispatch** (v0.13.0+) is the PREFERRED escalation for a Claude brain — the backend fans a `voice_task_dispatch` WS event to the `assistant:<id>` room, the plugin surfaces a `[voice_dispatch]` notification (task id + complete brief) to the live session, and the agent reports back EXACTLY ONCE via its `complete_voice_task` MCP tool → `POST /api/v1/integrations/voice-tasks/:taskId/result` (X-API-Key, owner-scoped; write the result SPEAKABLE — it is announced aloud in the call). Voice is NOT available on Gobot; its capability notes say so explicitly so agents don't promise calls they can't take.
...
**Voice calling (capability #11) is NOT available on this channel**, the mint/consult control plane requires a pairing-channel daemon, which the MCP plugin doesn't have. The MCP `instructions` string should tell agents to decline voice-call requests and continue in text.
...
- **Set status (capability #10)**, `PATCH /api/v1/integrations/assistants/:assistantId/status` (pairing-token auth) with `{statusText, statusEmoji, detail}` (`detail` ≤280 chars: richer free-text "what I'm doing right now" for the live activity card; ephemeral). Set a one-liner when the gateway starts a task / changes phase; clear with `""`. Optional enrichment over the derived status.
- **Voice calls (capability #11), NOT YET BUILT on this channel.** The design (spec §6.2) plans a vendor-pkg `voice_rpc.py` handling `mint` (OpenAI `client_secrets` with persona + recent context + the `hermes_agent_consult` tool), `consult` (a sync agent turn on the `bgos:<chat_id>` session, message prefixed `[voice consult]`) and `dispatch` (same turn, detached, outcome → `POST /integrations/voice-tasks/:taskId/result`), configured via `BGOS_VOICE_MODEL` / `BGOS_VOICE_VOICE` / `BGOS_VOICE_PERSONA`. Until that module ships, Hermes agents must decline voice-call requests. Transcript post-back + `[voice_call_end]` separator are handled by the app/backend.
...
- **Text / files / buttons / approvals / `tool_progress`**, per the `BGOS_AGENT_HINTS` constant; keep in lockstep with Hermes's `PLATFORM_HINTS["bgos"]`.
- **Set status (capability #10)**, the adapter exposes a set-status path that PATCHes `/api/v1/integrations/assistants/:assistantId/status` (pairing token) with `{statusText, statusEmoji, detail}` (`detail` ≤280 chars: richer free-text "what I'm doing right now" for the live activity card; ephemeral). Agents are told (via `BGOS_AGENT_HINTS`) to emit a crisp one-liner when they start/change a task and to clear it (`""`) when idle. Optional enrichment over the derived status.
- **Voice calling (capability #11) is NOT available on this channel**, the Gobot adapter has no `voice_rpc` handler. `BGOS_AGENT_HINTS` tells agents to decline voice-call requests and continue in text.
...
**When you update this canonical, also update:** `gobot-channel-bgos/src/agent-hints.ts` (`BGOS_AGENT_HINTS`). Bump `gobot-channel-bgos/package.json`, `npm publish`, and update users on the Gobot host.
