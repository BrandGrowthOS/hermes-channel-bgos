...
**Hermes spoken replies:** The `/voice` slash command persists the voice mode per chat. `/voice off` keeps replies text only, `/voice on` speaks replies to voice notes, and `/voice tts` speaks every reply. When the gateway speaks, the adapter posts one real voice bubble with `isAudioMessage: true`, audio, and the full spoken words in `text`, so the app renders the audio with the text underneath. First use on a fresh host downloads a speech model (about 150 MB). The adapter posts a **Voice setup** `event` card with real download progress under `eventMeta.source="voice_setup"` and `eventMeta.payload.progress`, whose fields are `stage`, `downloadedBytes`, and `totalBytes`, then PATCHes that card in place. Set `BGOS_VOICE_SETUP_CARD=off` to disable the setup card.
...
- **Hermes voice notes**, voice notes and audio attachments in `m4a`, `aac`, `mp3`, `ogg`, `opus`, `webm`, `wav`, and `flac` formats are downloaded by the adapter and handed to the gateway as voice messages, which routes them into the host's speech to text pipeline. Each transcript reaches the agent as a quoted line in the message text while the attachment link line stays available. Each file is capped at 25 MB and at most 5 audio files are routed per message. Oversized or undownloadable audio and old Hermes hosts fall back to the plain attachment link line. Set `BGOS_VOICE_NOTES=off` to disable transcription routing.
...
- **Spoken replies**, the user or app sets Hermes's persisted per-chat mode with `/voice off` (text only), `/voice on` (voice replies to voice notes), or `/voice tts` (every reply spoken). No agent-side syntax is needed. When the gateway speaks, `send_voice` posts one `isAudioMessage` voice bubble with the full spoken words in `text`, so the app renders audio with the text underneath. First use on a fresh host downloads a speech model (about 150 MB); the adapter posts a **Voice setup** `event` card with `eventMeta.source="voice_setup"` and real progress in `eventMeta.payload.progress` using `stage`, `downloadedBytes`, and `totalBytes`, then PATCHes it in place. Set `BGOS_VOICE_SETUP_CARD=off` to disable the card.
...
- **Ring the owner (live voice call)** (WIRED, this repo, 2026-07-27): embed a `[[BGOS_CALL]]short reason[[/BGOS_CALL]]` block in the reply. The adapter places the call as the agent that OWNS the chat (resolved per chat via `_style_assistant_id_for_chat`, never from config, because one daemon can serve several agents), strips the marker from the visible text, and still sends any remaining text as a normal message. One marker per turn rings once; the reason is capped at 200 chars to match the backend limit. A 409 `busy` means the owner is already on a call and NOTHING rang, so follow up in chat rather than retrying. Agents must NOT shell out to `curl` to ring: that trips the agent HOST's terminal approval layer, a different gate from BGOS approvals, and the owner sees a generic command prompt instead of a call.
...
- Memory panel: when enabled in the app, your owner can read, edit, and delete your MEMORY and USER notes and search past conversations from your agent profile. Panel edits land on disk immediately and apply from the next session. If a note changed unexpectedly, your owner likely edited it.
- **Meetings + turn resync (capability #13)**, NOT yet implemented. The Hermes WS bridge (`bgos_ws.py`) subscribes only to `inbound_message` / `callback_result` / `inbound_click` / `voice_rpc` / `skills_rpc` / `doctor_rpc` / `memory_rpc`; it does not consume the meeting turn-protocol events. When meeting support is added here, it MUST handle `meeting_state_resync` on (re)connect exactly as capability #13 describes (refresh turn state, act only when `currentSpeakerId` is this assistant, gate on `lastMessageId` for idempotency, never reply when not on the floor).
